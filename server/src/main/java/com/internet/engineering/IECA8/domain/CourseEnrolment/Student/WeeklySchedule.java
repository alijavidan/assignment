...
public class WeeklySchedule {
    ...
    private void ifAlreadyPassed(WeeklyScheduleItem weeklyScheduleItem) throws Exception {
        if (weeklyScheduleItem.getStudent().getGrades().stream().map(Grade::getCode).collect(Collectors.toList()).contains(weeklyScheduleItem.getOffering().getCode()))
    ...
    public void checkPrerquisites(WeeklyScheduleItem newWeeklyScheduleItem , Student student)  throws Exception{
        ...
        if (!studentGrades.stream().map(Grade::getCode).collect(Collectors.toList()).containsAll(prerequisites.stream().map(Prerequisite::getPrerequisite).collect(Collectors.toList())))
    ...
    private void unitCheck(Student student) throws Exception{
        ...
        CourseEnrolment.getInstance().getWeeklySchedule(student.getStudentId(), true).
                stream().
                collect(Collectors.toList()).
    ...
    public void handleSignedUps(Student student) throws Exception {
        relativeComplements(student.getSelectedCourses(),
            student.getLastSubmit())
                .stream()
                .filter(weeklyScheduleItem -> weeklyScheduleItem.getStatus().equals("finalized"))
                .forEach(w -> w.getOffering().signedUpInc());

        relativeComplements(student.getLastSubmit(),
    ...
    public void finalizeWeeklySchedule(Student student) throws Exception{
        ...
        CourseEnrolment.getInstance().getWeeklySchedule(student.getStudentId(), false).
                stream().
